...
def fetch_weather() -> dict[str, Any] | None:
    ...
    for i, day in enumerate(daily.get("time", [])[:3]):
        ...
        days.append({
            "date": day,
            "label": f"{DAYS_SHORT[date.weekday()]}. {date.day}",
            "full_label": f"{DAYS_LONG[date.weekday()]} {date.day} {MONTHS_LONG[date.month]}",
...
def render_weather_cards(days: list[dict[str, Any]], wind_dir: str = "E") -> str:
    """Render the 3 météo cards in one block to avoid regex replacing the first card repeatedly."""
    cards = []
    for idx, day in enumerate(days[:3]):
        label = html_lib.escape(str(day.get("label", ""))).upper()
        icon = html_lib.escape(str(day.get("icon", "⛅")))
        tmax = html_lib.escape(str(day.get("tmax", "28")))
        tmin = html_lib.escape(str(day.get("tmin", "24")))
        wind = html_lib.escape(str(day.get("wind", "25")))
        condition = html_lib.escape(str(day.get("condition", "")))
        today_class = " today" if idx == 0 else ""
        cards.append(
            f"""    <div class=\"wx-day{today_class}\">
     <div class=\"wd-day\">{label}</div>
     <div class=\"wd-icon\">{icon}</div>
     <div class=\"wd-temp\">{tmax}°</div>
     <div class=\"wd-range\">{tmin}° · {tmax}°</div>
     <div class=\"wd-wind\">{html_lib.escape(wind_dir)} {wind} km/h<br>{condition}</div>
    </div>"""
        )
    return "   <div class=\"wx-3days\">\n" + "\n".join(cards) + "\n   </div>"
...
def patch_html(raw_html: str, payload: dict[str, Any]) -> str:
    ...
    if weather:
        ...
        if d:
            html = sub(r'   <div class="wx-3days">.*?   </div>\n  </div>', render_weather_cards(d, weather.get("wind_dir", "E")) + "\n  </div>", html, count=1, flags=re.S)
