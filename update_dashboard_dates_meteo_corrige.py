...
def patch_html(raw_html: str, payload: dict[str, Any]) -> str:
    ...
    if weather:
        ...
        # Remplacement du bloc complet des 3 cartes météo.
        # Important : ne pas remplacer les .wd-day un par un, sinon le même bloc
        # peut être réécrit plusieurs fois et créer des dates dupliquées / inversées.
        cards = []
        for idx, day in enumerate(d[:3]):
            today_class = " today" if idx == 0 else ""
            cards.append(
                f'
    <div class="wx-day{today_class}">
'
                f'     <div class="wd-day">{day["label"]}</div>
'
                f'     <div class="wd-icon">{day["icon"]}</div>
'
                f'     <div class="wd-temp">{day["tmax"]}°</div>
'
                f'     <div class="wd-range">{day["tmin"]}° · {day["tmax"]}°</div>
'
                f'     <div class="wd-wind">{weather["wind_dir"]} {day["wind"]} km/h<br>{day["condition"]}</div>
'
                f'    </div>'
            )
        new_cards_block = '<div class="wx-3days">' + ''.join(cards) + '\n   </div>'
        html = sub(
            r'<div class="wx-3days">\s*<div class="wx-day today">.*?</div>\s*</div>\s*<div class="wx-day">.*?</div>\s*</div>\s*<div class="wx-day">.*?</div>\s*</div>\s*</div>',
            new_cards_block,
            html,
            count=1,
            flags=re.S,
